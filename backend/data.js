export default{
    inizio: {
        spegnimentoCentrale: "Credevi fosse così facile? Devi fare LogIn per spegnere la centrale.",
        exit: "Questo è uno dei pochi casi in cui spegnere e riaccendere non risolve il problema!",
    },
    login: {
        russa: "Президент не нанимал вас на эту работу, спецназовец Романов. Возвращайтесь, чтобы украсть ценную информацию у грязных капиталистов, которые угрожают нашей стране. Слава Арстотцке!\n\nIl Presidente non ti ha incaricata per questo lavoro, soldato speciale Romanov. Torna a sottrare preziose informazioni ai sporchi capitalisti che minacciano il nostro paese. Gloria ad Arstotzka!",
        ita: "Ciao Alfredo, Don Corleone la sta aspettando al cantiere. Ha un nuovo paio di scarpe da farti provare. Se lo farai atendere potrebbe essere l'ultim volta, ah no lo sarà lo stesso. Buona nuotata.",
        homer: "Homer, quante volte bisogna dirtelo che non puoi accedere al sistema? Di sicuro non dopo che hai rovesciato il caffè mentre inzuppavi una ciambella mandando in corto il quadro elettrico e rischiando di provocare un disastro nucleare!!! Torna a non fare le tua mansioni.",
    },
    mail: [
        {
            da: "risorse.umane@n-mail.val",
            a: "@tutti",
            oggetto: "Chi ha sporcato il bagno?",
            mail: "Invito gentilmente chiunque abbia usato il bagno al 3° piano, di presentarsi presso l'ufficio delle risorse umane per risponere del fatto che quando si producono più scorie del reattore 4 sarebbe gradito che venga tirata l'acqua!",
        },
        {
            da: "conferma.ordine@siberia.taiwan",
            a: "steven.olsson@gmail.com",
            oggetto: "Ordine Siberia.com",
            mail: "Gentile cliente, la informiamo che abbiamo spedito i suoi articoli. Il suo pacco arriverà in 14843 giorni lavorativi. La ringraziamo per aver scelto il nostro sito per i suoi acquisti. \n\nDettagio ordine: \n-Mutande con pizzo nere taglia XXL:\t\tx7\t19,99€ \n-Pelliccia copripollice in pettirosso:\t\tx1\t73,45€ \n-Dizionario Mandarino/Ghanese per espeerti:\tx1\t3,89€",
        },
        {
            da: "ufficio.tecnico@n-mail.val",
            a: "steven.olsson@n-mail.val",
            oggetto: "Cambio combinazione",
            mail: "La informiamo che la combianazione per il riposizionamneto delle barre del reattore 3 è stato cambiato.\nLa nuova combinazione è: \n",
        },
        {
            da: "pierfranco@n-mail.val",
            a: "@tutti, ivan.giardiniere@n-main.val",
            oggetto: "Festa a sorpresa",
            mail: "Ricordo a tutti che prossimo lunedì è il compleanno di Ivan il giardiniere. La festa a sorpresa si terrà quel giorno dopo il turno del pomeriggio all' 1:30. Mi raccomando, massima segretezza!",
        },
        {
            da: "info@zadrainterni.it",
            a: "",
            oggetto: "Arredamento per la casa, moderno e su misura in Trentino-Alto Adige",
            mail: "Raccontare una storia attraverso le mani di un uomo.\nEsplorare le infinite possibilità della materia pensando con il cuore.\nAttendere con ansia il momento di rientrare a casa\nper rivedere i propri cari e ascoltare il profumo del legno naturale.\n…non vendiamo semplici arredi ma un modo di sognare!\n\nIl design più ricercato si fonde all'umanità del legno,\narrediamo secondo i vostri sogni...\n\nTelefono/fax: +39 0463 468470\nFacebook: facebook.com/Rustiklegno/\n\nVia del Plan del Sant, 8 38012 Mollaro, Predaia TN",
        },
    ],
    note:[
        {
            nome: "nota 1",
            data: "28/06/2021",
            nota: "La signora Romanov è alquanto sospetta. Questa mattina mi ha fatto domande molto insitenti riguardo la mia famiglia e sul sistema di sicurezza della centrale."
        },
        {
            nome: "nota 2",
            data: "01/07/2021",
            nota: "Continuo a pensare di essere l'unico che lavora in questa centrale. La russa continua a scrivere lettere e a parlare al telefono, penso alla famiglia, parla solamente nella sua lingua e non si capisce niente. Il giapponese se ne stà nel suo ufficio notte e giorno. L'italiano è già tanto che arrivi per mezzogiorno. Homer oggi ha mangiato 19 ciambelle."
        },
        {
            nome: "nota 3",
            data: "23/06/2021",
            nota: "Oggi Homer è riuscito a mangiare 28 ciambelle in meno di un'ora!! Non smette mai di stupirmi."
        },
        {
            nome: "nota 4",
            data: "18/05/2020",
            nota: "Da comprare: \n-Pane \n-Salame \n-Bondola \n-Uova \n-Patate \n-Prosecco \n-Caramelle Rossana \n-Formai"
        },
        {
            nome: "nota 5",
            data: "11/06/2021",
            nota: "Questa mattina mi è caduta la tessera della console di comando all'interno del reattore 3! Spero non mi scoprano, non ho idea di come tirarla fuori..."
        },
        {
            nome: "nota 6",
            data: "05/06/2021",
            nota: "Credo che il nuovo staggista giapponese sia un'impostore!! L'ho visto usare una calcolatrice!! Esistono le calcolatrici in giappone? Non fanno tutto a mente?? Homer ha mangiato 12 ciambelle questa mattina."
        },
    ],
    orari:[
        {
            mansione: "LogIn",
            ora: "08:30",
            codice: "001"
        },
        {
            mansione: "Controllo orari, mail, note",
            ora: "08:45",
            codice: "010"
        },
        {
            mansione: "Controllo barre plutonio reattore 3",
            ora: "09:30",
            codice: "011"
        },
        {
            mansione: "Pausa pranzo",
            ora: "15:00",
            codice: "100"
        },
        {
            mansione: "Controllare Homer",
            ora: "15:15",
            codice: "101"
        },
        {
            mansione: "Riposizionamento barre plutonio reattore 3",
            ora: "15:30",
            codice: "110"
        },
        {
            mansione: "Fine turno",
            ora: "22:00",
            codice: "111"
        },
    ]
};
