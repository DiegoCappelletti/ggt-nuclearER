export default{
    inizio: {
            spegnimentoCentrale: "Credevi fosse così facile? Devi fare LogIn per spegnere la centrale.",
            exit: "Questo è uno dei pochi casi in cui spegnere e riaccendere non risolve il problema!",
        },
    login: {
            russa: "Il Presidente non ti ha incaricata per questo lavoro, soldato speciale Romanov. Torna a sottrare preziose informazioni ai sporchi capitalisti che minacciano il nostro paese. Gloria ad Arstotzka! Президент не нанимал вас на эту работу, спецназовец Романов. Возвращайтесь, чтобы украсть ценную информацию у грязных капиталистов, которые угрожают нашей стране. Слава Арстотцке!",
            ita: "",
            homer: "Homer, quante volte bisogna dirtelo che non puoi accedere al sistema? Di sicuro non dopo che hai rovescito il caffè mentre inzuppavi una ciambella mandando in corto il qudro elettrico e rischiando di provocare un disaastro nucleare!!! Torna a non fare le tua mansioni.",
        },
};
